...
한국어와 영어로 각각 검색한 결과로 부터 한국어 결과를 얻기 위해서는 영어로 검색한 결과를 한국어로 번역하여 한국어로 검색한 결과와 함께 LLM에 Context로 제공하여야 합니다. 한국어/영어 번역은 Propmt를 활용하여 LLM으로 수행할 수 있습니다. 따라서, 영어 질문으로 RAG의 지식저장소를 검색한 결과들을 한국어로 번역하기 위해 여러번의 LLM 요청이 필요합니다. 이를 순차적(sequencial)하게 처리하면 사용자의 질문과 답변사이의 지연 시간이 늘어나서 사용성이 나빠지므로, 여기에서는 Multi-Region LLM을 활용하여 다수의 LLM에 번역을 요청하여 시간을 단축하는 방법을 소개합니다.
